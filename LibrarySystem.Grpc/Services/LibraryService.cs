...
public class LibraryService : Library.LibraryBase
{
    ...
    {
        var command = new CreateBookCommand(request.Title, request.Author, request.PublicationYear, request.Pages,
            request.TotalCopies);
    ...
    public override async Task<BookResponse> UpdateBook(UpdateBookRequest request, ServerCallContext context)
    {
        try
        {
            var command = new UpdateBookCommand(request.Id, request.Title, request.Author, request.PublicationYear,
                request.Pages, request.TotalCopies);
            var result = await _sender.Send(command, context.CancellationToken);
            return MapToResponse(result);
        }
        ...
        ServerCallContext context)
    {
        try
        {
            var result = await _sender.Send(new BorrowBookCommand(request.BookId, request.BorrowerId),
    ...
    public override async Task<LendingActivityResponse> ReturnBook(ReturnBookRequest request, ServerCallContext context)
    {
        try
        {
            var result = await _sender.Send(new ReturnBookCommand(request.LendingActivityId),
                context.CancellationToken);
            return new LendingActivityResponse
            {
                Id = result.Id, BookId = result.BookId, BorrowerId = result.BorrowerId,
        ...
        ServerCallContext context)
    {
        try
        {
            ...
            {
                TotalCopies = result.TotalCopies, BorrowedCopies = result.BorrowedCopies,
        ...
        ServerCallContext context)
    {
        var result =
            await _sender.Send(
                new GetTopBorrowersQuery(request.StartDate.ToDateTime(), request.EndDate.ToDateTime(), request.Count),
                context.CancellationToken);
        ...
        GetUserLendingHistoryRequest request, ServerCallContext context)
    {
        var result =
            await _sender.Send(
                new GetUserHistoryQuery(request.BorrowerId, request.StartDate.ToDateTime(),
                    request.EndDate.ToDateTime()), context.CancellationToken);
        ...
        response.History.AddRange(result.Select(h => new UserLendingHistoryItem
        {
            ...
            ReturnedDate = h.ReturnedDate.HasValue
                ? Timestamp.FromDateTime(h.ReturnedDate.Value.ToUniversalTime())
                : null
        }));
        return response;
    }
    ...
    private static BookResponse MapToResponse(BookDto dto)
    {
        ...
        {
            Id = dto.Id, Title = dto.Title, Author = dto.Author, PublicationYear = dto.PublicationYear,
            Pages = dto.Pages, TotalCopies = dto.TotalCopies
        };
    }
}