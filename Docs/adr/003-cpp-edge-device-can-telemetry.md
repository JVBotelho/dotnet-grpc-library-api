...
9. **Embedded delivery.** The C++ kiosk is **statically cross-compiled to `aarch64` (musl)** so it runs on a Raspberry-Pi-class target, reusing the toolchain approach proven in `skewrun`. A `docker compose` profile brings up the whole heterogeneous fleet (C# core + WAF + Postgres + C++ kiosk) with one command. For cross-platform portability the compose path feeds the kiosk over a **UDP transport shim** carrying byte-identical CAN-FD frames; the **real `SocketCAN`/`vcan0` path is exercised in CI** rather than in the local compose fleet (see *Transport portability* under Lessons Learned).
...
- **Transport Portability — UDP Shim Locally, Real `vcan0` in CI**: `SocketCAN`/`vcan0` is a Linux-kernel facility that is **absent from the default WSL2 and Docker Desktop kernels**, so `modprobe vcan` fails out-of-the-box on a Windows or macOS reviewer's machine. Requiring it for the local demo would break the "runs in 30 seconds, hardware-free" property. The `CanReader` therefore accepts two frame sources behind **one decode path**: a real `SocketCAN` `vcan0` socket (`CAN_RAW_FD_FRAMES`, MTU 72) and a cross-platform **UDP transport shim** that carries **byte-identical `canfd_frame` payloads** (DBC-encoded via `cantools`, packed to the 72-byte wire layout). `SignalCodec`'s bit-level extraction is identical in both modes — the UDP path is a *transport*, not a simulation of CAN content. The `docker compose` fleet uses the UDP shim for portability; the **genuine `vcan0`/CAN-FD path is proven end-to-end in CI** (`scripts/e2e-test.sh` on GitHub Actions installs `linux-modules-extra-$(uname -r)`, configures `vcan0` at MTU 72, and asserts the full `vcan0 → C++ → mTLS gRPC → C# → PostgreSQL → STOP_MOTOR` round-trip). If a runner cannot load the `vcan` module, the script logs the reason and falls back to UDP rather than failing silently. This makes "CAN-FD shown, not claimed" verifiable from a green CI run without anyone recompiling a kernel.
- **Direct Exposure of gRPC mTLS (WAF Bypass)**: The edge device (Kiosk) requires end-to-end mTLS (Client Certificate validation by Kestrel). An attempt to use an L7 WAF (Caddy/Coraza) to perform L4 TCP pass-through proved to be an anti-pattern and caused instability. As a result, the architecture bypasses the WAF for port `8443`, exposing the backend `library-grpc` Kestrel server directly to the edge. While aligned with Zero-Trust (network is untrusted, auth happens at the workload), this is a conscious trade-off. In a true enterprise environment, instead of direct exposure, a dedicated Network Load Balancer (NLB) for L4 traffic (e.g., HAProxy, Envoy, AWS NLB) should be placed at the edge to absorb DDoS attacks and connection exhaustion before traffic reaches the application server.
